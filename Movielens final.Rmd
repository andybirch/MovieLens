---
title: "MovieLens - Movie recommendation project"
author: "Andy Birch"
date: "22/05/2021"
output: 
  pdf_document:
...
```{r download and prepare initial data, include  = FALSE, cache = TRUE}
...
Data Science course. The objective is to build a machine learning model that can accurately predict the rating a user will give a specific movie. 

The analysis makes use of the MovieLens 10M dataset, which contains over ten million movie reviews provided by the GroupLens research group at the Department of Computer Science and Engineering at the University of Minnesota[^1]. We will use 90% of these reviews to develop and test models, before making a final model selection. We will then make predictions for the other 10% of reviews and compare to actual rating given.
The target level of accuracy for the final model is to achieve a root mean squared error (RMSE) of  `r target` or below.

This exercise is an example of a so called *recommender system*, which have a wide range of real world applications. In this case it could allow a movie streaming service to make personalised recommendations for what movies to watch next, in the hope that it builds user engagement and decreases customer churn. Recommender systems are also used by retailers to suggest other products to purchase and social media platforms to decide what content to display.

This report provides an overview of the dataset and the key fields, discussing their potential use in our model. A series of potential models are introduced and tested, with the final model selected when its performance is strong enough for us to confidently believe that the final assessment will beat the target RMSE. The final RMSE performance is then presented and potential further work discussed.
...
Further, when looking at the highest rated movies (those with an average rating of four or greater), we can see a significant majority have a low number of reviews:
...
This needs to be considered when building the model, as we should have less confidence in a high average rating for a movie with a small number of reviews. Failing to adjust for this issue could lead to overfitting, which can lead to the selection of a model that has good accuracy for the training set, but is poor for the validation set. 
...
The most complex combination for *genre* has `r max(str_count(genres$genres,"\\|")) + 1` different elements:
...
There are `r nrow(genres)` unique values in the *genre* field and the most popular have a large number of reviews (n_reviews) across many movies (n_movies):
...
In theory the *genre* could provide useful predictive power, if a user likes a comedy movie for example, it seems reasonable to expect that they will like other comedy movies. However the definition of a movie's *genre* is largely subjective and whilst the individual genre elements are quite intuitive (drama, comedy etc), it is very difficult to quantify their existence in a movie. 
...
Firstly the *edx* dataset is split into two parts, 90% of records are randomly selected for the training set, the remaining 10% are held back to test each model. Next we need to define the loss function to test each model, the course instructions stipulated that root mean squared error (RMSE) be used:
...
After the initial data preparation, the model parameters need to be tuned using cross validation. The values recommended by the package authors were used initially[^3]:

```{r recosystem tuning, echo = FALSE}
...
\pagebreak
```{r RMSE for recosystem, echo = FALSE, out.width = '50%', fig.align = 'center', fig.cap = 'RMSE results for each model', fig.show="hold"}
results <- rbind(results, 
...
The RMSE for the validation set is `r loss(validation$final_pred_capped, validation$rating)`, which is significantly better than the target of `r target`, therefore the model has achieved our stated objective.
...
The simplest models calculated and applied bias terms for the *movieId* and for the *userId*, they produced good results, but were not accurate enough to beat the target RMSE. Our final model used matrix factorisation to identify similarities between movies and between users to draw on more rating information to make predictions. This significantly improved accuracy and comfortably beat the target RMSE.
